...
function Request() {
  ...
  useEffect(() => {
    async function getCountryNameFromCode(id: string): Promise<Country> {
      try {
        const req = await fetch(`https://restcountries.com/v3.1/alpha/${id}`);
        ...
        throw new Error(`failed to fetch countrycode ${id}: ` + e);
      }
    }
    async function getUser(): Promise<User> {
      try {
        ...
        const countryinfo = await getCountryNameFromCode(userdata.nat).catch(
          e => {
            return {
              name: e + " landia",
              emoji: "🏳️",
    ...
    <div>
      {user ? (
        <div className="usercard">
          ...
          <p>{user.email}</p>
          <p>{user.nat}</p>
          <img src={user.image.toString()} alt="User Avatar" />
        </div>
      ) : (
        <p>Loading...</p>
      )}
